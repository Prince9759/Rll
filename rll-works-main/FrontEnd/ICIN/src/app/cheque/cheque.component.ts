...
updateAccount(account:any)
     {
        ...
        this.crequest=account.crequest;
        this.cstatus=account.cstatus;
...
updateDataFromDb(){
  ...
  for (let i = 0; i < this.accounts.length; i++) {
    if (this.accounts[i].aid==this.aid) {
       account = {abalance:this.accounts[i].abalance+this.depositedamount, aid:this.aid, uname:"hello", uemailid:this.accounts[i].uemailid, anumber:this.accounts[i].anumber, atype:this.accounts[i].atype, password:this.accounts[i].password,crequest:"generated",cstatus:"pending"};
       this.msg="Request Generated";
